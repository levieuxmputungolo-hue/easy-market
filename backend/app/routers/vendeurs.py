...
ALLOWED_ID_CARD_TYPES = {"image/jpeg", "image/png", "image/webp"}
...
async def register_vendeur(
    ...
    if id_card.content_type not in ALLOWED_ID_CARD_TYPES:
        raise HTTPException(400, f"Type de fichier non autorisé : {id_card.content_type}. Formats acceptés : JPEG, PNG, WebP")
...
async def login_vendeur(data: dict, response: Response):
    ...
    if v.get("password", "").startswith("$2b$") == False:
        import hashlib
